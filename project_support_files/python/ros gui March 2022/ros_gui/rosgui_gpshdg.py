...
class Display_Sensor_1(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.sub = rospy.Subscriber("/heading_deg", Float32, self.callback_sensor_1)
        self.sensor_1_data = tk.IntVar()

        # configure the root window
        self.title('GPS Heading')
        self.resizable(0, 0)
        # width x height = 1920 x 1080 (in pixels) of the Ubuntu laptop
        self.geometry('225x70+1350+300')
        self['bg'] = 'black'
    ...
    def callback_sensor_1(self, data):   
        n = 1 # the number of decimal places to show
        s = '{}'.format(data.data)
        if 'e' in s or 'E' in s:
