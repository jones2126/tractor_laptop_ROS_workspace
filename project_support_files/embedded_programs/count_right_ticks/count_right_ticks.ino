/*
 * Description: Calculate the accumulated ticks for right wheel using the AS5048 magnetic encoder
 * another example at: Website: https://automaticaddison.com
 * When moving forward the tick count is positive
 */
...
void setup() {
  Serial.begin(115200);
  Serial.print("right tick counter program has started ");
  Serial.println();
  Serial.println();      
  Wire.begin();
...
void loop() {
  ...
  if (currentMillis - prev_time_poll_AS5048B >= poll_AS5048B_Interval) {
    ...
    last_position = current_position;        
    right_wheel_tick_count.data = total_ticks_moved;   
    }  
